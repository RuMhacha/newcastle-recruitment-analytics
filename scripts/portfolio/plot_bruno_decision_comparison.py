...
def create_chart(bruno: pd.DataFrame) -> None:
    """Compare initial succession scores with final risk-adjusted scores."""

    chart_data = bruno.sort_values(
        "Risk_Adjusted_Score",
        ascending=True,
    )
    ...
    ax.set_title(
        "Decision modelling reshapes the Bruno Guimarães succession shortlist",
        fontsize=16,
        fontweight="bold",
        pad=18,
    )
    ...
    output_file = output_dir / "bruno_decision_comparison.png"
